...
Status Config::validate() {
    ...
    if (this->election_tick <= this->heartbeat_tick) {
        return Status::invalid_argument("election tick must be greaer than heartbeat tick");
    }
    // if(!this->storage){
    ...
    if (this->read_only_option == ReadOnlyLeaseBased && !this->check_quorum) {
        return Status::invalid_argument(
            "check_quorum must be enabled when read_only_option is ReadOnlyLeaseBased");
    }
    return Status::ok();
};
} // namespace kv