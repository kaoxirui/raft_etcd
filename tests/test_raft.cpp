...
void testLeaderElection(bool preVote) {
...

    {
        ...
        Test t{.network = std::make_shared<Network>(cfg, peers),
...

    {
        ...
        std::vector<RaftPtr> peers{nullptr, entsWithConfig(cfg, std::vector<uint64_t>{1}),
                                   entsWithConfig(cfg, std::vector<uint64_t>{1}),
                                   entsWithConfig(cfg, std::vector<uint64_t>{1, 1}), nullptr};
        Test t{.network = std::make_shared<Network>(cfg, peers),
...
TEST(raft, LearnerElectionTimeout) {
    auto n1 = newTestLearnerRaft(1, std::vector<uint64_t>{1}, std::vector<uint64_t>{2}, 10, 1,
                                 std::make_shared<MemoryStorage>());
    auto n2 = newTestLearnerRaft(2, std::vector<uint64_t>{1}, std::vector<uint64_t>{2}, 10, 1,
                                 std::make_shared<MemoryStorage>());
...
void testVoteFromAnyState(proto::MessageType vt) {
    for (auto st = (size_t)RaftState::Follower; st <= RaftState::PreCandidate; st++) {
        auto r = newTestRaft(1, std::vector<uint64_t>{1, 2, 3}, 10, 1,
                             std::make_shared<MemoryStorage>());
        r->term_ = 1;
...
TEST(raft, LogReplication) {
...

    {
        ...
        tests.push_back(Test{
            .network = std::make_shared<Network>(peers),
            .msgs = msgs,
            .wcommitted = 2,
        });
    }
...
TEST(raft, DuelingCandidates) {
    auto a =
        newTestRaft(1, std::vector<uint64_t>{1, 2, 3}, 10, 1, std::make_shared<MemoryStorage>());
    auto b =
        newTestRaft(2, std::vector<uint64_t>{1, 2, 3}, 10, 1, std::make_shared<MemoryStorage>());
    auto c =
        newTestRaft(3, std::vector<uint64_t>{1, 2, 3}, 10, 1, std::make_shared<MemoryStorage>());
...
TEST(raft, DuelingPreCandidates) {
    auto cfgA =
        newTestConfig(1, std::vector<uint64_t>{1, 2, 3}, 10, 1, std::make_shared<MemoryStorage>());
    auto cfgB =
        newTestConfig(2, std::vector<uint64_t>{1, 2, 3}, 10, 1, std::make_shared<MemoryStorage>());
    auto cfgC =
        newTestConfig(3, std::vector<uint64_t>{1, 2, 3}, 10, 1, std::make_shared<MemoryStorage>());
    cfgA.pre_vote = true;
    cfgB.pre_vote = true;
    cfgC.pre_vote = true;
