...
bool entry_cmp(const std::vector<proto::EntryPtr> &left,
               const std::vector<proto::EntryPtr> &right) {
...
Config newTestConfig(uint64_t id, std::vector<uint64_t> peers, uint32_t election,
                     uint32_t heartbeat, StoragePtr storage) {
    Config c;
    c.id = id;
    c.peers = peers;
...
RaftPtr entsWithConfig(ConfigFunc configFunc, std::vector<uint64_t> terms) {
    ...
    auto cfg = newTestConfig(1, std::vector<uint64_t>(), 5, 1, storage);
    if (configFunc) {
        configFunc(cfg);
    }
...
RaftPtr newTestRaft(uint64_t id, std::vector<uint64_t> peers, uint64_t election, uint64_t heartbeat,
                    StoragePtr storage) {
...
RaftPtr newTestLearnerRaft(uint64_t id, std::vector<uint64_t> peers, std::vector<uint64_t> learners,
                           uint64_t election, uint64_t heartbeat, StoragePtr storage) {
    Config c = newTestConfig(id, peers, election, heartbeat, storage);
    c.learners = learners;
    c.max_inflight_msgs = 256;
    Status status = c.validate();
...
template <>
struct hash<connem> {
    std::size_t operator()(const connem &c) const { return boost::hash_value(tie(c.from, c.to)); }
};
...
public:
    explicit BlackHole()
        : Raft(newTestConfig(0, std::vector<uint64_t>{1, 2, 3}, 1, 2,
    ...
    virtual Status step(proto::MessagePtr msg) { return Status::ok(); }
};

struct Network {
    explicit Network(const std::vector<RaftPtr> &peers) : Network([](Config &c) {}, peers) {}
    ...
    void ignore(proto::MessageType t) { ignorem[t] = true; }
        ...
        for (proto::MessagePtr m : msgs) {
            ...
            switch (m->type) {
                ...
                default: {
                    ...
                    if (n < perc * 100) {
                        LOG_DEBUG("drop message, %lu, %lu, %s", m->from, m->to,
                                  proto::msg_type_to_string(m->type));
                        continue;
                    }
                }
            }
