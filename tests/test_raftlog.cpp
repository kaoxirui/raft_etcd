    ...
    for (size_t i = 0; i < tests.size(); ++i) {
        ...
        if (test.wpanic) {
            ...
            if (ok && !test.ents.empty()) {
                std::vector<proto::EntryPtr> ents;
                l.slice(l.last_index() - test.ents.size() + 1, l.last_index() + 1,
                        std::numeric_limits<uint64_t>::max(), ents);
...
TEST(raftlog, committo) {
    ...
    tests.push_back(
        Test{.commit = 4, .wcommit = 0, .wpanic = true}); // commit out of range -> panic
...
TEST(raftlog, stabletosnap) {
    ...
    std::vector<Test> tests;
    {
        std::vector<proto::EntryPtr> ents;
        tests.push_back(
            Test{.stablei = snapi + 1, .stablet = snapt, .newEnts = ents, .wunstable = snapi + 1});
    }
    {
        std::vector<proto::EntryPtr> ents;
        tests.push_back(
            Test{.stablei = snapi, .stablet = snapt, .newEnts = ents, .wunstable = snapi + 1});
    }
    {
        std::vector<proto::EntryPtr> ents;
        tests.push_back(
    ...
    }
    {
        std::vector<proto::EntryPtr> ents;
        tests.push_back(Test{
            .stablei = snapi + 1, .stablet = snapt + 1, .newEnts = ents, .wunstable = snapi + 1});
    }

    {
        std::vector<proto::EntryPtr> ents;
        tests.push_back(
            Test{.stablei = snapi, .stablet = snapt + 1, .newEnts = ents, .wunstable = snapi + 1});
    }
    {
        std::vector<proto::EntryPtr> ents;
        tests.push_back(Test{
...

    {
        ...
        tests.push_back(
            Test{.stablei = snapi + 1, .stablet = snapt, .newEnts = ents, .wunstable = snapi + 2});
    }
    {
        std::vector<proto::EntryPtr> ents;
        ents.push_back(newEntry(snapi + 1, snapt));
