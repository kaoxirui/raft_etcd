...
public:
    ...
    std::string status_string() const {
        char buffer[64];
        int n = snprintf(buffer, sizeof(buffer), "committed=%lu,applied=%lu,unstable,offset=%lu,unstable.entries=%lu", committed_,
                         applied_, unstable_->offset_, unstable_->entries_.size());
