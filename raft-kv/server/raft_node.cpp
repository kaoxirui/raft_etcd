...
RaftNode::RaftNode(uint64_t id, const std::string &cluster, uint16_t port)
    : port_(port), pthread_id_(0), timer_(io_service_), id_(id), last_index_(0),
      conf_state_(new proto::ConfState()), snapshot_index_(0), applied_index_(0),
      storage_(new MemoryStorage()), snap_count_(defaultSnapCount) {
            ...
            case proto::EntryConfChange: {
                ...
                try {
                    msgpack::object_handle oh =
                        msgpack::unpack((const char *)entry->data.data(), entry->data.size());
                    oh.get().convert(cc);
...
void RaftNode::entries_to_apply(const std::vector<proto::EntryPtr> &entries,
    ...
    if (first > applied_index_ + 1) {
        LOG_FATAL("first index of committed entry[%lu] should <= progress.appliedIndex[%lu]+1",
...
void RaftNode::maybe_trigger_snapshot() {
    ...
    redis_server_->get_snapshot(
        std::move([&promise](const SnapshotDataPtr &data) { promise.set_value(data); }));
...
void RaftNode::main(uint64_t id, const std::string &cluster, uint16_t port) {
    ...
    g_node = std::make_shared<RaftNode>(id, cluster, port); //创建共享指针，指向新的raftnode实例

    g_node->transport_ =
        Transport::create(g_node.get(), g_node->id_); //创建传输层，并与当前节点关联
    std::string &host = g_node->peers_[id - 1];       //获取当前节点的主机信息
    g_node->transport_->start(host);                  //启动传输层
    //添加集群中其他节点
