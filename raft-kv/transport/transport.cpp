...
public:
    explicit TransportImpl(RaftServer *raft, uint64_t id) : raft_(raft), id_(id) {}
    ~TransportImpl() final {
        //检查io_thread_是否可加入（即线程是否已经启动但尚未被加入）
        if (io_thread_.joinable()) {
            //线程可加入，调用join方法，等待线程完成
            io_thread_.join();
    ...
    void remove_peer(uint64_t id) final { LOG_WARN("no impl yet"); }
