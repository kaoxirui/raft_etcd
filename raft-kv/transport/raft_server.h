...
public:
    ...
    virtual void process(proto::MessagePtr msg,
                         const std::function<void(const Status &)> &callback) = 0;
...
public:
    ...
    static std::shared_ptr<IoServer> create(void *io_service, const std::string &host,
                                            RaftServer *raft);
};
