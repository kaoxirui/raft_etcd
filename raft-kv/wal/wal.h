...
public:
    ...
    static bool search_index(const std::vector<std::string> &names, uint64_t index,
                             uint64_t *name_index);

private:
    explicit WAL(const std::string &dir) : dir_(dir), enti_(0) {
        memset(&start_, 0, sizeof(start_));
    }

    void handle_record_wal_record(WAL_type type, const char *data, size_t data_len, bool &matchsnap,
